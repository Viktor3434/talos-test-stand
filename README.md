...
## 📁 Структура репозитория
    ...
    │   └── README.md
    │
    ├── 01-talos-test-stand-up/                 # Логика поднятия Talos-кластера (Vagrant + Python)
...
Если вы хотите автоматически развернуть весь стенд (VM + Talos + ArgoCD + приложения), следуйте инструкциям из директории `01-talos-test-stand-up`. Этот процесс включает:
